...
// A product's `image` is a full Cloudinary URL, not a site-relative path.
// Legacy/rare records could still hold an old relative path, so this stays
// safe either way instead of always concatenating the site origin onto it
// (which previously produced broken URLs like
// "https://www.hydel.co.inhttps//res.cloudinary.com/...").
function absoluteProductImageUrl(image: string | undefined): string {
  if (!image) return 'https://www.hydel.co.in/hydel.png';
  if (/^https?:\/\//i.test(image)) return image;
  return `https://www.hydel.co.in${image.startsWith('/') ? '' : '/'}${image}`;
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return {
    ...
    twitter: {
      ...
      title: title,
      description: description,
      images: [absoluteProductImageUrl(product.image)],
    },
    robots: {
      index: true,
      follow: true,
...
export default async function ProductDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const productSchema = {
    ...
    "image": [
      absoluteProductImageUrl(product.image)
    ],
    "brand": {
      "@type": "Brand",
    ...
    "offers": {
      ...
      // Always ~1 year out instead of a fixed past date, so this never ships
      // an already-expired price validity window.
    ...
    // No "gtin" here on purpose: we don't have real, registered GTINs for
    // these products, and Google's structured-data guidelines treat
    // fabricated identifiers as invalid/spammy data.
    "additionalProperty": [
      {
        "@type": "PropertyValue",
        "name": "Customizable",
    ...
    // No "aggregateRating" here on purpose: the previous version hardcoded
    // an identical 4.8/127-review rating on every single product page. That
    // isn't real review data, and Google's review-snippet policy treats
    // fabricated/duplicated ratings as a violation that can get rich
    // results disabled sitewide. Add this back only once genuine per-product
    // reviews exist.
  };
